...
//import {getCredText} from './UserDetail.js';
...
class LeaderboardItem extends Component {
  ...
  render() {
    const {user, index, userTypeToShow, checkHasAccess, noAccessHandler} = this.props;
      ...
      <React.Fragment>
        <tr>
          <td className="displayFlex alignCenter positionRel">
            <span className="internalBorder green hidden height60px" />
            {(index + 1 > 3) && (
              <span className="greyText verticallyCenter marginLeft14"><span className="number">{index + 1}</span></span>
            )}
            {(index + 1 == 1) && (
              <span className="goldText verticallyCenter marginLeft10"><i className="fas fa-trophy" /><span className="number hidden">1</span></span>
            )}
            {(index + 1 == 2) && (
              <span className="silverText verticallyCenter marginLeft10"><i className="fas fa-trophy" /><span className="number hidden">2</span></span>
            )}
            {(index + 1 == 3) && (
              <span className="bronzeText verticallyCenter marginLeft10"><i className="fas fa-trophy" /><span className="number hidden">3</span></span>
            )}
          </td>
          <td className="textLeft paddingR20 maxWidth320px">
            <div className="displayFlex">
              <Avatar userID={user.uid} isAnon={false} isGroupFlex userName={user.fname} showAsCircle picSize={360}/>
              <div className="max2Lines">
                <div className="darkGreyText fontSize14">
                  ...
                  {(userTypeToShow == '1' && user.isU18 != true) && (
                  ...
                  {(userTypeToShow == '2' || (userTypeToShow == '1' && user.isU18 == true)) && (
                    <span className="bold">{nameToShow}</span>
                  )}
                </div>
                <div className="fontSize14 mediumGreyText">Role and Company go here</div>
              </div>
            </div>
          </td>
          ...
          <td className="alignCenter">
            <svg className="pointsIcon width18px height18px filter-grey" viewBox="0 0 256 256">
            ...
            </svg> <span className="number">{user.numAnswers}</span>
          </td>
          <td className="alignCenter">
            <span className="fontSize12 paddingR5 pointsIcon"><i className="fas fa-pencil-alt" /></span> <span className="number">{user.numGenerals}</span>
          </td>
          <td className="alignCenter">
            <span className="fontSize12 paddingR5 pointsIcon"><i className="fas fa-user-friends" /></span> <span className="number">{(userTypeToShow == '0' || userTypeToShow == '2') ? user.numMentees : user.numMentors}</span>
          </td>
        </tr>
      </React.Fragment>
    )
