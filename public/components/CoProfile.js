...
class CoProfile extends React.Component {
  ...
  componentDidMount() {
    ...
    const company = {
      ...
      subscriptionstatus: 0 // 0 = 'none', '1' = 'free , '2' = premium
    }
    if(company != null){
      updateDocumentTitle("Explore " + company.name + " careers & advice - Prospela.com")
    }
    const loggedInUID = '8'
  ...
  handleSubmitUnsubscribe = () => {
    ...
    if (wantsToLeave == 1) {
      this.setState({
        ...
        subscriptionStatus: 1, // Set back to '1' i.e free version of profile
      })
    } else { // user wanted to stay
      this.setState({
        isUnsubscribing: false,
        updateUnsubscribeSuccess: true,
        subscriptionStatus: 2, // Keep as '2' i.e premium version of profile
      })
    }
  }
        ...
        <React.Fragment>
          ...
          {(approvalStatus == '1' || approvalStatus == '2') && (
            <React.Fragment>
              ...
              </svg> <span className="profileClaimStatus">{isPageManager ? 'Upgrade' : 'Claimed'}</span>
              <div className={"tooltiptext coProfile below top25 padding10 normalLineheight" + (isMobile ? " last signUpPage" : "")}>
                <div>{isPageManager ? 'Someone from this company manages this profile. Upgrade to enjoy job listings, enhanced employer branding and more' : 'Someone from this company manages this profile.'}</div>
                {isPageManager && (
                  <div className="marginTop10 lightPurpleText">
  ...
  render() {
    const {tabToView, isPageManager, subscriptionStatus, wantsToLeave, isUnsubscribing, updateUnsubscribeSuccess, isMobile, showClaimFreeProfileFormModal, showSuccessModal} = this.state
    const {userRole, isLoggedIn} = this.props;
    const company = {
      coid: '1',
      approvalstatus: 3,
    ...
    var loggedInUID = '8'
    let urlText, companyItem

    companyItem = getCompanyDeets(company.coid)
    urlText = companyItem.urlText
    ...
    metaAdder('property="og:title"', "Explore " + company.name + " careers & advice - Prospela.com")
    metaAdder('name="title"', "Explore " + company.name + " careers & advice - Prospela.com")
          ...
          <div className="mainAndSideContainer">
            <div className="title-blankPage marginBottom20">
              ...
              <div className="greyText fontSize12 marginBottom20 noBold">
                <i className="fas fa-home" /> &gt; Companies &gt; {company.name}
              </div>
                ...
                <div className="chatItemFlexContainer qTitle qaPage">
                  <div>
                    ...
                    <div className="qDetail normalLineheight fontSize13 noBold marginBottom20 breakWord">
                      Discover {company.name}: learn directly from real employees, and explore work-life reality
                    </div>
                    <button type="button" className="button-unstyled qDetail fontSize14 marginBottom20 breakWord noBold" name="leaderboard" onClick={(e) => this.updateTabToView(e)} >
                      <i className="fas fa-user-friends" /> XXX questions
                      {company.experts.length > 0 && (
                        <React.Fragment>
                          ...
                          <span> {company.experts.length} employee experts</span>
                        </React.Fragment>
                      )}
                    </button>
                  </div>
            ...
            <div className="groupdash-menuBar borderBtm borderGrey commPage">
              ...
              <Link to={{pathname: companyURLending + "/jobs-and-opportunities", state: {prevPath: window.location.pathname}}}>
                <button type="button" name="jobs" onClick={(e) => {this.updateTabToView(e)}} className={'button-unstyled groupdash-menuBtn' + (tabToView == 'jobs' ? ' tabActive' : '')}>Jobs & Ops</button>
              </Link>
            </div>
