...
function checkHasLessonAccess(lid, courseDetail, isLoggedIn, user, usersCoursesCids, modules) {
  /*
  Potential responses:
  - Y
  - N-needsPremiumAccess
  - N-lessonNotPartOfCourse
  - N-hasNotBoughtCourse
  - N-notLoggedInAndNotOpenLesson
  */

  const openLessons = courseDetail && courseDetail.openlessons
  const premiumLessons = courseDetail && courseDetail.premiumlessons
  let allCourseLessonsWIP, allCourseLessons

  // Is an open lesson
  if (openLessons && openLessons.includes(lid)) {
    return "Y"

  } else if (isLoggedIn == true) {

    // Check if user has access to this course
    if (usersCoursesCids && usersCoursesCids.includes(courseDetail.cid)) {
      // Double check this lesson is in that course
      allCourseLessonsWIP = modules && modules.map(x => x.lessons)
      allCourseLessons = allCourseLessonsWIP && allCourseLessonsWIP.flat()

      if (allCourseLessons && allCourseLessons.includes(lid)) {

        // Check if is premium lesson (if so, does user have access)
        if (premiumLessons && premiumLessons.includes(lid)) {
          return (user && user.premiumlessons.includes(lid)) ? "Y" : "N-needsPremiumAccess"
        } else return "Y"
      } else return "N-lessonNotPartOfCourse"

    } else return "N-hasNotBoughtCourse"

  } else return "N-notLoggedInAndNotOpenLesson"
}
...
{/*  <span className="lessonCompletedNotif channel">
    <Checkbox
      labelClassName="checkbox-container menuNotif"
      className="SubmitMatch-input"
      spanClassName="checkmark disabled"
      defaultChecked={false}
      disabled
    />
  </span> */}

function getLockedOrCompletedIndicator(isLocked, isCompleted) {

  if (isLocked) {
    return (
      <span className="lessonCompletedNotif channel isLocked goldText">
    ...
    return (
      <span className="lessonCompletedNotif channel isLocked greyText">
...
export {checkHasLessonAccess, getLockedOrCompletedIndicator, getLoremIpsumText, removeHash, isIE, isEdge, isiOS, isURL, escapeHTML, metaAdder, getIcon, getUnreadIndicator, showNotifFavicon, hideNotifFavicon, getChannelAbout, whichBrowser, checkMobile, checkDevice, DateCalc, TimeCalc, percentageCircle, ChevronDown, ChevronUp, X, Check, PenIcon, LoadingSpinner, sortTable, monthDiff};
